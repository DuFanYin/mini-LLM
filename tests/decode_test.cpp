...
TEST(DecodeTest, SampleFromLogitsIsDeterministicForSameSeed) {
    ...
    const uint32_t id_a = sample(logits_a, 1.0f, r1);
    const uint32_t id_b = sample(logits_b, 1.0f, r2);
    EXPECT_EQ(id_a, id_b);
}
...
TEST(DecodeTest, SampleVeryLowTemperatureMatchesArgmax) {
    ...
    std::mt19937 rng(1);
    const uint32_t sampled = sample(logits, 1e-8f, rng);
    const uint32_t greedy = argmax(logits_saved);
