...
void project_logits_into(const std::vector<float>& hidden_states, size_t seq_len, size_t d_model,
    ...
    kernel::gemm_nt(hidden_states.data(), lm_head.data(), out.logits.data(), seq_len, vocab_size, d_model);
}
...
void project_logits_steps_into(const float* hidden_states, size_t seq_len, size_t d_model,
    ...
    kernel::gemm_nt(gathered_workspace.data(), lm_head.data(), out.logits.data(), n, vocab_size, d_model);
}
...
void logits_from_hidden_row(std::span<const float> hidden_row, const std::vector<float>& lm_head, size_t vocab_size,
    ...
    kernel::gemm_nt(hidden_row.data(), lm_head.data(), logits_out.data(), 1, vocab_size, d_model);
}
...
uint32_t argmax_from_hidden(std::span<const float> hidden_row, const std::vector<float>& lm_head, size_t vocab_size,
                            size_t d_model) {
    std::vector<float> logits;
    logits_from_hidden_row(hidden_row, lm_head, vocab_size, d_model, logits);
...
uint32_t sample_from_hidden_row(std::span<const float> hidden_row, const std::vector<float>& lm_head,
