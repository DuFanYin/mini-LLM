...
void TrainWorkspace::prepare(const model::MiniLlm& m) {
    ...
    if (grad.layers.size() != num_layers || grad.vocab_size != weights.vocab_size ||
        grad.token_embedding.size() != weights.token_embedding.size() || grad.output_projection.size() != weights.output_projection.size()) {
...
StepMetrics train_step(model::MiniLlm& m, const std::vector<uint32_t>& token_ids,
    ...
    m.forward_train(token_ids, workspace.tapes, workspace.hidden_states.data());
    ...
    engine::project_logits_steps_into(workspace.hidden_states.data(), token_ids.size(), m.d_model(),
                                      ...
                                      weights.output_projection, weights.vocab_size, workspace.logits, workspace.gathered_hidden);
    ...
    kernel::backward_output_projection(workspace.hidden_states.data(), ce.probs.data(), ce.targets.data(), ce.steps.data(),
    ...
    kernel::backward_hidden(ce.probs.data(), ce.targets.data(), ce.steps.data(), ce.valid_steps, weights.output_projection.data(),
